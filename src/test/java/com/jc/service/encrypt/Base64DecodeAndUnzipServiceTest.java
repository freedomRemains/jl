...
public class Base64DecodeAndUnzipServiceTest extends TestBase {
	...
	void beforeEach() {
		...
		new FileUtil().createDirIfNotExists(OUTPUT_PATH + "ZipAndBase64EncodeServiceTest");
		new FileUtil().createDirIfNotExists(OUTPUT_PATH + "ZipAndBase64EncodeServiceTest/unzip");
	}
	...
	void test01() {
		...
		try {
			input.putString("encodeResultFilePath", OUTPUT_PATH + "ZipAndBase64EncodeServiceTest/" + "dirToZipEncode.txt");
			service.doService(input, output);
			fail();
		} catch (BusinessRuleViolationException e) {
	...
	private GenericParam getNormalInput(String targetDirOrFile) {
		...
		// エンコードを行う
		var input = new GenericParam();
		input.putString("targetDirOrFile", RESOURCE_PATH + "service/encrypt/ZipServiceTest/" + targetDirOrFile);
		input.putString("outputDir", OUTPUT_PATH + "ZipAndBase64EncodeServiceTest");
		var output = new GenericParam();
		var service = new ZipAndBase64EncodeService();
		service.doService(input, output);

		// 出力パラメータをデコードの入力パラメータとするため、必要な入力パラメータを設定する
		output.putString("outputDir", OUTPUT_PATH + "ZipAndBase64EncodeServiceTest/unzip");
	...
	private void assertNormal(GenericParam output, String targetDirOrFile) {
		...
		String outputDir = OUTPUT_PATH + "ZipAndBase64EncodeServiceTest/unzip/";
	...
	private boolean existsInOutputDir(String targetFileNamePart) {
		...
		File outputDir = new File(OUTPUT_PATH + "ZipAndBase64EncodeServiceTest");
