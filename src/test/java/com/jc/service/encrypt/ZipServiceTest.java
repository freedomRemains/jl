 ...
 */
public class ZipServiceTest extends TestBase {
	...
	void test01() {
		...
		try {
			input.putString("targetDirOrFile", OUTPUT_PATH + "ZipServiceTest/" + "dirToZip");
			service.doService(input, output);
			fail();
		} catch (BusinessRuleViolationException e) {
	...
	void test05() {
		...
		// カバレッジ(拡張子ありのファイルを圧縮)
		var input = getNormalInput("dirToZip");
		input.putString("targetDirOrFile", RESOURCE_PATH + "service/encrypt/ZipServiceTest/fileToZip.txt");
		var output = new GenericParam();
		var service = new ZipService();
		service.doService(input, output);
