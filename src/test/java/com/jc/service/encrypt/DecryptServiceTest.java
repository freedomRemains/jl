...
public class DecryptServiceTest extends TestBase {
	...
	void test01() {
		...
		try {
			input.putString("secretKeyFilePath", OUTPUT_PATH + "ZipServiceTest/" + "dirToZip_encrypt.key");
			service.doService(input, output);
			fail();
		} catch (BusinessRuleViolationException e) {
		...
		try {
			input.putString("ivFilePath", OUTPUT_PATH + "ZipServiceTest/" + "dirToZip_encrypt.iv");
			service.doService(input, output);
			fail();
		} catch (BusinessRuleViolationException e) {
		...
		try {
			input.putString("encryptResultFilePath", OUTPUT_PATH + "ZipServiceTest/" + "dirToZip_encrypt.bin");
			service.doService(input, output);
			fail();
		} catch (BusinessRuleViolationException e) {
