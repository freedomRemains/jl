...
public class GetAllTableDropSqlServiceTest extends TestBase {
	...
	private void assertNoFileOutput() {
		...
		String outputPath = OUTPUT_PATH + "dbmng/" + dbName + "/30_sql/20_auto_created/";
		assertFalse(new File(outputPath + "DROP_MGNRGRP.txt").exists());
		assertFalse(new File(outputPath + "DROP_MGNRGRP.txt").length() > 0);
		assertFalse(new File(outputPath + "DROP_MGNRKEYVAL.txt").exists());
		assertFalse(new File(outputPath + "DROP_MGNRKEYVAL.txt").length() > 0);
		assertFalse(new File(outputPath + "DROP_TACCOUNT.txt").exists());
		assertFalse(new File(outputPath + "DROP_TACCOUNT.txt").length() > 0);
		assertFalse(new File(outputPath + "DROP_TSCR.txt").exists());
		assertFalse(new File(outputPath + "DROP_TSCR.txt").length() > 0);
		assertFalse(new File(outputPath + "DROP_TSCRELM.txt").exists());
		assertFalse(new File(outputPath + "DROP_TSCRELM.txt").length() > 0);
	}
}
