 ...
 * アプリケーション内部例外
 */
class ApplicationInternalExceptionTest extends TestBase {
	...
	void test01() {
		...
		// 例外クラスを引数とするパターンのテスト
		try {
			throw new ApplicationInternalException(new Exception("一般例外"));
	...
	void test02() {
		...
		// 文字列を引数とするパターンのテスト
		try {
			throw new ApplicationInternalException("文字列例外");
		} catch (RuntimeException e) {
