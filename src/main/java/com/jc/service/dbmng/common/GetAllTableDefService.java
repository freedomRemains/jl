 ...
 * テーブルの定義情報を取得するクラスです。
 */
public class GetAllTableDefService implements ServiceInterface {
	...
	private void getTableDef(GenericParam input, GenericParam output) {
		...
		if (Cu.isNotEmpty(tableNameListFilePath)) {
			...
			for (String tableName : tableNameList) {
				...
				getTableDefByTableName(input, output, tableName);
			}
	...
	private void getTableDefByTableName(GenericParam input, GenericParam output, String tableName) {
		...
		GetTableDefService getTableDefService = new GetTableDefService();
		getTableDefService.doService(input, output);
