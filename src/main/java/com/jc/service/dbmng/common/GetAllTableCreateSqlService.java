 ...
 */
public class GetAllTableCreateSqlService implements ServiceInterface {
	...
	private void getTableSql(GenericParam input, GenericParam output) {
		...
		if (Cu.isNotEmpty(tableNameListFilePath)) {
			...
			for (String tableName : tableNameList) {
				...
				getTableCreateSqlByTableName(input, output, tableName);
			}
	...
	private void getTableCreateSqlByTableName(GenericParam input, GenericParam output, String tableName) {

		// テーブル名に基づいて、サービスを呼び出す
		String tableDefFilePath = input.getString("dirPath") + PATH_DELM + input.getString("defPath") + PATH_DELM
				+ tableName + ".txt";
		GetTableCreateSqlService getTableCreateSqlService = new GetTableCreateSqlService();
		input.putString("tableName", tableName);
		input.putString("tableDefFilePath", tableDefFilePath);
		getTableCreateSqlService.doService(input, output);
	}
}
