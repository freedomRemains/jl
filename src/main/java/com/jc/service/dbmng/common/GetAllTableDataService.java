 ...
 * テーブルデータを取得するクラスです。
 */
public class GetAllTableDataService implements ServiceInterface {
	...
	private void getTableData(GenericParam input, GenericParam output) {
		...
		if (Cu.isNotEmpty(tableNameListFilePath)) {
			...
			for (String tableName : tableNameList) {
				...
				getTableDataByTableName(input, output, tableName);
			}
	...
	private void getTableDataByTableName(GenericParam input, GenericParam output, String tableName) {

		// テーブル名に基づいて、サービスを呼び出す
		String tableDefFilePath = input.getString("dirPath") + PATH_DELM + input.getString("defPath") + PATH_DELM
				+ tableName + ".txt";
		GetTableDataService getTableDataService = new GetTableDataService();
		input.putString("tableName", tableName);
		input.putString("tableDefFilePath", tableDefFilePath);
		getTableDataService.doService(input, output);
	}
}
